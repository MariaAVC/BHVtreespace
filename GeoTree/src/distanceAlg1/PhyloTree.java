...
//						newickString = newickString + ":" + d6o.format(leafEdgeAttribs[i]);
	...
	public void addToEdgeAttributes(double [] coords, boolean negate) {
		...
		// switch sign of each coordinate if necessary
		if (negate) {
		...
		int i = 0;		// coords array index
		
		// INTERIOR EDGES
		// Assumes interior edge coordinates are in same order as when read tree in from Newick
		// (which they will be if the coordinates are generated by the logmap method)
		for (PhyloTreeEdge edge : this.edges) {
			// extract the relevant coordinates
			double [] edgeCoords = new double[dimAttrib];
			// j is the index for edgeCoords array; add i to index in coords array
			for (int j = 0; j<dimAttrib; j++) {
				edgeCoords[j] = coords[i];
				i++;
			}
			edge.getAttribute().add(new EdgeAttribute(edgeCoords));
		}
		
		// LEAF EDGES
		for (EdgeAttribute leafAttrib: this.leafEdgeAttribs) {
			double [] leafCoords = new double[dimAttrib];
			// j is the index for leafCoords array; add i to index in coords array
			for (int j = 0; j<dimAttrib; j++) {
				leafCoords[j] = coords[i];
				i++;
			}
